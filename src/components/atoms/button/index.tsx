    ...
    <TButton
      className={cn(
        'cursor-pointer bg-coral text-xl text-white py-0 px-6 w-fit font-normal shadow-none hover:shadow-none hover:bg-light-coral focus:border-4 focus:border-charcoal rounded-sm focus:rounded-lg h-12 disabled:bg-gray',
        {
          ...
          'bg-transparent text-charcoal hover:bg-transparent hover:text-seafoam-green px-[0.25rem] py-[0.25rem] focus:border-0': variant === 'outlined',
        },
        className,
      )}
      onClick={(e) => onClick(e)}
      ...
      {...rest}
    >
      <span className="flex capitalize gap-[0.125rem] w-fit items-center">
        {children}
        {iconId && (
